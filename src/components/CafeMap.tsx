...
function makePopupContent(cafe: CafePin, isLoyal: boolean, isSaved: boolean): string {
  ...
  return `<div class="strudl-map-popup">
    ...
    <div class="strudl-map-popup-actions">
      <button class="strudl-map-visit-btn" onclick="window.__strudlVisitCafe()">Visit café</button>
...
export default function CafeMap({ loyalCafeNames, savedCafeIds = [], onSaveToggle, fullScreen = false, height }: Props) {
  ...
  useEffect(() => {
    ;(window as any).__strudlVisitCafe = () => navigate('/discover')
    ...
    return () => {
      delete (window as any).__strudlVisitCafe
  ...
  useEffect(() => {
    ...
    VIENNA_CAFES.forEach((cafe) => {
      ...
      const marker = L.marker([cafe.lat, cafe.lng], { icon: makeMarkerIcon(state) })
        .addTo(map)
        .bindPopup(makePopupContent(cafe, isLoyal, isSaved), { maxWidth: 260 })
  ...
  return (
    ...
    >
      ...
      <button
        ...
        className="absolute left-3 bottom-3 z-[1000] bg-white rounded-xl shadow-[0_4px_12px_rgba(0,0,0,0.15)] border border-[#dadada] p-2.5 active:scale-95 transition-transform"
