...
export default function ShareModal({ open, onClose, favoriteCafes, userName }: Props) {
  ...
  return (
    <>
      ...
      {/* Bottom sheet */}
      <div
        className={`fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-[430px] z-[2001] bg-white rounded-t-3xl max-h-[92svh] overflow-y-auto transition-transform duration-300 ${open ? 'translate-y-0' : 'translate-y-full'}`}
      >
        {/* Drag handle */}
        <div className="flex justify-center pt-3 pb-1">
          <div className="w-9 h-1 rounded-full bg-[#dadada]" />
        </div>

        {/* Header */}
        <div className="px-5 pt-2 pb-4 flex items-start justify-between">
          <div className="flex-1 pr-4">
            <h2 className="text-xl font-black text-[#0f0f0f]" style={{ letterSpacing: '-0.03em' }}>
              Share your favorite cafés
            </h2>
            <p className="text-[#5f5f5f] text-sm mt-1 leading-relaxed">
              Send your top spots to a friend — they don't need Strudl to view the list.
            </p>
          </div>
          <button
            onClick={onClose}
            className="w-8 h-8 rounded-full bg-[#f6f6f6] flex items-center justify-center flex-shrink-0 mt-0.5 active:scale-95 transition-transform"
          >
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#5f5f5f" strokeWidth="2.5" strokeLinecap="round">
        ...
        <div className="px-5 pb-10 flex flex-col gap-6">
          ...
          <div>
            <p className="text-sm font-semibold text-[#0f0f0f] mb-3">What to share</p>
            <div className="flex bg-[#f6f6f6] border border-[#dadada] rounded-2xl p-1 mb-3">
              <button
            ...
            {shareType === 'city' && cities.length > 1 && (
              <div className="relative mb-2">
                <select
                  ...
                  className="w-full bg-[#f6f6f6] border border-[#dadada] rounded-xl px-4 py-3 text-sm font-medium text-[#0f0f0f] appearance-none pr-10"
                >
                  {cities.map(c => (
            ...
            <p className="text-xs text-[#5f5f5f]">
              <span className="font-semibold text-[#0f0f0f]">{filteredCafes.length}</span>{' '}
          ...
          <div>
            ...
            <div className="relative">
              <textarea
                ...
                rows={3}
                className="w-full bg-[#f6f6f6] border border-[#dadada] rounded-xl px-4 py-3 text-sm text-[#0f0f0f] resize-none leading-relaxed"
          ...
          {/* Privacy disclaimer */}
          <div className="bg-[#f6f6f6] rounded-xl px-4 py-3 flex gap-3 items-start">
          ...
          {/* Section 3: Share via */}
          <div>
            <p className="text-sm font-semibold text-[#0f0f0f] mb-4">Share via</p>
            <div className="flex gap-2 justify-around">
              ...
              <button onClick={handleCopy} className="flex flex-col items-center gap-1.5 active:scale-95 transition-transform">
                <div className={`w-14 h-14 rounded-2xl border flex items-center justify-center transition-colors duration-200 ${
                  copied ? 'bg-[#D97706] border-[#D97706]' : 'bg-[#f6f6f6] border-[#dadada]'
                }`}>
                  {copied
                ...
                <span className="text-[11px] text-[#5f5f5f] font-medium">{copied ? 'Copied!' : 'Copy link'}</span>
              </button>

              {/* WhatsApp */}
              <button onClick={handleWhatsApp} className="flex flex-col items-center gap-1.5 active:scale-95 transition-transform">
                <div className="w-14 h-14 rounded-2xl border border-[#dadada] bg-[#f6f6f6] flex items-center justify-center">
                ...
                <span className="text-[11px] text-[#5f5f5f] font-medium">WhatsApp</span>
              </button>
              ...
              <button onClick={handleSMS} className="flex flex-col items-center gap-1.5 active:scale-95 transition-transform">
                ...
                <span className="text-[11px] text-[#5f5f5f] font-medium">Message</span>
              </button>
              ...
              <button onClick={handleEmail} className="flex flex-col items-center gap-1.5 active:scale-95 transition-transform">
                ...
                <span className="text-[11px] text-[#5f5f5f] font-medium">Email</span>
              </button>
              ...
              {typeof navigator !== 'undefined' && 'share' in navigator && (
                <button onClick={handleNativeShare} className="flex flex-col items-center gap-1.5 active:scale-95 transition-transform">
                  ...
                  <span className="text-[11px] text-[#5f5f5f] font-medium">Share</span>
                </button>
              )}
            </div>
          </div>
